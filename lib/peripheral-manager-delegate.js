...
CBPeripheralManagerDelegate.addMethod('peripheralManager:central:didExchangeMTU:',
                                    'v@:@@@', function ($self, $_cmd, $peripheralManager, $deviceUUID, $mtu) {
  var centralIdentifier = $util.toUuidString($deviceUUID);
  var mtu               = $util.toInt($mtu);

  debug('peripheralManagercentralDidExchangeMTU:%s %d', centralIdentifier, mtu);

  process.nextTick(function(centralIdentifier, mtu) {
    this.emit('mtuChange', centralIdentifier, mtu);
  }.bind(mapDelegate($self), centralIdentifier, mtu));
});
...
CBPeripheralManagerDelegate.addMethod('peripheralManager:central:didSubscribeToCharacteristic:',
  ...
  var maximumUpdateValueLength = $central('maximumUpdateValueLength');
  var characteristicIdentifier = $util.identifierFor$MutableAttribute($characteristic);

  debug('peripheralManagerCentralDidSubscribeToCharacteristic:%s %d %s', centralIdentifier, maximumUpdateValueLength, characteristicIdentifier);

  process.nextTick(function(centralIdentifier, maximumUpdateValueLength, characteristicIdentifier) {
    this.emit('subscribe', centralIdentifier, maximumUpdateValueLength, characteristicIdentifier);
  }.bind(mapDelegate($self), centralIdentifier, maximumUpdateValueLength, characteristicIdentifier));
});
...
CBPeripheralManagerDelegate.addMethod('peripheralManager:didUpdateValueForCharacteristic:',
                                    'v@:@@', function ($self, $_cmd, $peripheralManager, $attributeID) {
  var characteristicIdentifier = $util.toInt($attributeID);

  debug('peripheralManagerDidUpdateValueForCharacteristic:%s %s', characteristicIdentifier);

  process.nextTick(function(characteristicIdentifier) {
    this.emit('valueUpdate', characteristicIdentifier);
  }.bind(mapDelegate($self), characteristicIdentifier));
});
